...
const ProjectStepper = ({ steps, currentStep }: Props) => {
  return (
    <ul className="w-full border rounded-md border-slate-300 flex flex-col md:flex-row md:justify-between md:px-4  divide divide  divide-slate-300 md:divide-y-0">
      {steps.map((step) => (
        <li
          className={`py-2 px-2 flex items-center gap-3 opacity-80 hover:opacity-100 cursor-pointer ${
